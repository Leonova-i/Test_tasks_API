...
class TestUserAuth:
    ...
    def setup(self):
        ...
        assert "x-csrf-token" in response1.headers, "There is no CSRF in token headers in the response"
    ...
    def test_negative_check_user(self, condition):
        ...
        assert user_id_from_check_method == 0, f"User is authorize auth in the response with {condition}"
